...
setInterval(() => {
    d3.json("http://localhost:3000/getStreamedLanguages").then(data => {;
        if(data.length != 0){
            ...
            console.log(data)
            d3.select('#language_stream').select("svg").remove()
            var svg = d3.select('#language_stream')
              .append("svg")
            ...
            y = d3.scaleLinear()
              .domain(([0, (d3.max(data, function(element) { return element.count*1.05}))])) //set height of Y-axis to max value of elements
